...
public class CurriculumVitaeController {
    ...
    @PostMapping("/candidate/create-cv/{cadidate}")
    public String creatCv(Model model,@PathVariable(value = "cadidate") int cadidate, @ModelAttribute(value = "cv") @Valid CurriculumVitae cv, 
            BindingResult r) {
        if (r.hasErrors()) {
            return "createCV";
        }
        if (curriculumVitaeService.addCV(cv,cadidate) ==true) {
            return ("index");
        }
        return "createCV";
    }
